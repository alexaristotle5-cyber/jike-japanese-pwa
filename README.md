...
当前公网版暂时屏蔽邮箱 Magic Link，改用固定账号密码登录。前端只保存固定账号到内部 Supabase Auth 邮箱的映射，不保存密码；密码由 Supabase Auth 校验。
...
- 登录方式当前是固定账号密码，适合小范围使用；如果公开范围变大，建议改成正式邮箱/手机号登录。
